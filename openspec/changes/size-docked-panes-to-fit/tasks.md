...
- [ ] 7.6 Re-judge the affected surfaces; their recorded verdicts describe the old behaviour. Not done here on purpose — this is a human attended-review judgement (does the new appearance actually look right to a reviewer), not a fact this pass can determine or should decide on your behalf. `all-tools-docked` and `three-tools-tabbed` are the surfaces whose prior verdicts predate this change.
