...
- [x] 2.1 Add `scripts/pages/verify-graph.mjs`, run before any build work.
...
- [x] 2.2 Validate that `.ua/knowledge-graph.json` exists, parses, and carries
      the node/edge/layer/tour shape the dashboard requires; fail naming the
      file otherwise.
- [x] 2.3 Provenance: read `project.gitCommitHash`; fail when absent,
      malformed, or unreachable in history; warn (and continue) when it is an
      ancestor of the deployed commit.
- [x] 2.4 Regression: compare per-type edge counts and node count against the
...
- [x] 2.5 Excluded paths: fail when any node's `filePath` matches a directory
      or file excluded by `.ua/.understandignore`.
- [x] 2.6 Accept an override for the regression gate only, so a deliberate
      scope reduction can be published without weakening the other checks.

## 3. Publish workflow

- [x] 3.1 Add `.github/workflows/pages.yml` triggered by pushes to `main` that
      touch the published graph artifacts, plus `workflow_dispatch`. No
      `pull_request` trigger.
- [x] 3.2 Serialise with a `concurrency` group and `cancel-in-progress: false`
      — a superseded deployment must not leave the site half-updated.
- [x] 3.3 Check out with full history; the provenance and regression checks
      both need it.
- [x] 3.4 Check out the Understand-Anything dashboard at a pinned commit SHA
      into a separate path.
- [x] 3.5 Build `@understand-anything/core`, then build the dashboard with
      `vite.config.demo.ts` and `--base=/PracticeTakes/`.
- [x] 3.6 Stage `knowledge-graph.json`, `meta.json`, and `config.json` into the
      build output. Do not stage `fingerprints.json` or `intermediate/`.
- [x] 3.7 Copy the upstream MIT `LICENSE` into the output and link it from the
      entry page.
- [x] 3.8 Scan the staged output with gitleaks before upload; a detection fails
      the job.
- [x] 3.9 Upload and deploy, with `pages: write` and `id-token: write` scoped
      to the deploy job only.

## 4. Verify

- [x] 4.1 Build locally against the pinned dashboard commit and serve the
      output with a plain static file server; confirm the dashboard renders and
      fetches the graph with no token.
- [x] 4.2 Confirm the regression gate fires: run it against a graph with an
