...
Define an application-owned, immutable, engraving-independent representation of a score — its entities, its invariants, its single musical time base, and the ownership and threading rules that let rendering, playback, and session code share one instance safely.
