...
Defines what must be observed at runtime rather than asserted in prose — data
races, memory errors, undefined behaviour, and real-time safety — and the rule
that a check nobody reads is not a check.

Static analysis reads code; these checks watch a program run. The audio-thread
contract in particular is enforceable only by execution: a rule that the audio
callback must not allocate, lock, or block means nothing until a test drives
that callback under a tool that would notice.
