...
- **THEN** `scripts/secrets/test_secrets_manager.py` is among the tests
...
files under `scripts/` or the check's own workflow file, so changes limited to
the C++ application, the services workspace, or documentation do not run it.

#### Scenario: Change outside scripts/ does not trigger the check
...
(`scripts/release/version.py`) SHALL have automated tests covering version
