...
Defines the durable store behind a verification run: what it holds about the machine, the captures, the reviewer's decisions, ingested measurements and automated results, how it is migrated, and how a run is exported to the record the release gate reads.
