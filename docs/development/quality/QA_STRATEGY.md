...
- **Performance**: one Catch2 benchmark exists today
  (`src/tests/features/analysis/tuner/PitchDetectorTests.cpp`, tag `[.benchmark]`, comparing FFT vs.
...
(`src/tests/features/analysis/tuner/PitchDetectorTests.cpp`, `[.benchmark]` tag) but it's never run in
...
Run it locally with `python3 tools/scripts/quality/run_coverage.py` (C++),
`npm run test:coverage` from `src/services/`, and
`python3 tools/scripts/quality/run_python_coverage.py`.
...
That is why the report has a second half: `tools/scripts/quality/coverage_sources.py`
