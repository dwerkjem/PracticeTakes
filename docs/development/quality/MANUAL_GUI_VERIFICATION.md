...
```bash
uv run tools/scripts/manual_gui --quick     # does it still work?
uv run tools/scripts/manual_gui --full      # before a release
uv run tools/scripts/manual_gui --full --geometry-sweep
uv run tools/scripts/manual_gui --check     # no prompts; proves the harness works
```
...
```bash
.venv/bin/python3 tools/scripts/manual_gui --quick
```
...
`tools/scripts/release/check_manual_verification.py`: `src/`, `CMakeLists.txt`,
`cmake/`, `packaging/`, `vcpkg.json`. `docs/`, `src/tests/`, `tools/scripts/`, and
`src/services/` are not — none of them changes the desktop binary. `VERSION` is
...
Surfaces are data, in `tools/scripts/manual_gui/surfaces.py`. Adding one is an edit to
