...
`tests/` mirrors `src/`, so a test's path names the source directory it covers.

```
src/services/score/TempoMap.h
tests/services/score/TempoMapTests.cpp
```
...
the matching one under `tests/` rather than putting the test at the top level.
...
Both `src/` and `tests/` are on the test target's include path, so:
...
`tests/support/` holds fixtures used by tests in more than one area. Those
...
`scripts/quality/check_test_layout.py`, with the reason. That list is the
documentation of what is exempt; keeping it short is the point.

## The check

`scripts/quality/check_test_layout.py` enforces two rules and runs in CI on any
change to `tests/`, `src/`, or `scripts/`:

1. No test file sits directly at the root of `tests/`. The root is where files
...
```bash
python3 scripts/quality/check_test_layout.py
```
