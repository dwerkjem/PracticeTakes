...
# The tests moved under src/, so they match the pattern above and the rglob in
# find_compiled_sources. They are the measurement, not the thing measured.
TEST_ROOT_PREFIX = "src/tests/"
...
def read_test_target_sources(cmake_lists: Path) -> set[str]:
    ...
    return {
        path
        for path in SOURCE_ENTRY_PATTERN.findall(match.group(1))
        if not path.startswith(TEST_ROOT_PREFIX)
    }
...
def find_compiled_sources(source_root: Path) -> set[str]:
    ...
    for path in source_root.rglob("*"):
        if path.is_file() and path.suffix in COMPILED_SUFFIXES:
            relative = f"src/{path.relative_to(source_root).as_posix()}"
            if not relative.startswith(TEST_ROOT_PREFIX):
