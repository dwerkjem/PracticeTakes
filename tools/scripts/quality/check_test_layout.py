#!/usr/bin/env python3
"""Check that ``src/tests/`` mirrors ``src/``.

A test's path is meant to name the source directory it covers, so
``src/services/score/TempoMap.h`` is tested by
``src/tests/platform/score/TempoMapTests.cpp``. That only stays true if something
...
1. No test file sits directly at the root of ``src/tests/``. The root is where
...
# Directories under ``src/tests/`` that intentionally mirror nothing in ``src/``,
...
def is_exempt(relative_directory: Path) -> bool:
    """Whether a directory under ``src/tests/`` is outside the mirror by design."""
...
def find_problems(tests_root: Path, source_root: Path) -> list[str]:
    ...
    for path in sorted(tests_root.rglob("*")):
        ...
        if directory == Path("."):
            problems.append(
                f"{display(path)} sits at the root of src/tests/. "
                f"Move it beside the mirror of the file it covers, "
                f"for example src/tests/platform/score/ for src/platform/score/."
            )
            continue
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--tests-root",
        type=Path,
        default=REPOSITORY_ROOT / "src" / "tests",
        help="Directory holding the tests (defaults to the repository's src/tests/).",
    )
    parser.add_argument(
        "--source-root",
        type=Path,
    ...
    if problems:
        print("src/tests/ does not mirror src/:", file=sys.stderr)
