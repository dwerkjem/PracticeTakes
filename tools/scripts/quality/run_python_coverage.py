...
Runs ``tools/scripts/run_tests.py`` under ``coverage`` and writes a machine-readable
...
def main(argv: list[str] | None = None) -> int:
    ...
    # `tools/scripts/` is the measured tree, and the test files themselves are
    ...
    run = subprocess.run(
        [
            *common,
            ...
            "--source",
            str(REPOSITORY_ROOT / "tools" / "scripts"),
            "--omit",
            "*/test_*.py",
            str(REPOSITORY_ROOT / "tools" / "scripts" / "run_tests.py"),
        ],
        cwd=REPOSITORY_ROOT,
        check=False,
    )
