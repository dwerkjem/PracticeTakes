...
class GateTests(unittest.TestCase):
    ...
    def test_test_changes_do_not_invalidate_a_run(self) -> None:
        """src/tests/ does not affect the shipped binary."""
        self.write_record()
        self.commit("src/tests/ThingTests.cpp", "// tests\n")
    ...
    def test_a_packaging_change_since_the_run_blocks(self) -> None:
        self.write_record()
        self.commit("tools/packaging/control", "Package: x\n")

        with self.assertRaises(gate.GateFailure) as caught:
            self.check()

        self.assertIn("tools/packaging/", str(caught.exception))
...
class ReleaseAffectingPathTests(unittest.TestCase):
    ...
    def test_the_binary_inputs_are_all_listed(self) -> None:
        for path in ("src/", "CMakeLists.txt", "tools/packaging/"):
