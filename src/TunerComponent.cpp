...
void TunerComponent::applyThemeToControls()
{
    ...
    displayModeBox.setColour(juce::ComboBox::backgroundColourId,
                             palette.control);
    displayModeBox.setColour(juce::ComboBox::textColourId,
                             palette.foreground);
    displayModeBox.setColour(juce::ComboBox::outlineColourId,
                             palette.outline);
    displayModeBox.setColour(juce::ComboBox::arrowColourId,
                             palette.foreground);
...
void TunerComponent::updateAdvancedSettingsVisibility()
{
    advancedSettingsButton.setButtonText(
        areAdvancedSettingsExpanded ? "Advanced settings  v"
                                    : "Advanced settings  >");
...
void TunerComponent::updateGraphControlAvailability()
{
    ...
    clearGraphButton.setVisible(
        areAdvancedSettingsExpanded && isGraphSelected);
}
...
int TunerComponent::controlAreaHeight() const
{
    ...
    return displaySelectorHeight
        + gapBelowDisplaySelector
        + advancedButtonHeight
...
juce::String TunerComponent::statusText() const
{
    ...
    return juce::String(displayedFrequency, 1) + " Hz   "
        + centsSign + juce::String(displayedCents, 1) + " cents";
}
...
void TunerComponent::writeInputSamplesToFifo(const float* inputSamples,
                                              int numSamples)
{
...
void TunerComponent::drainAudioFifo()
{
    ...
    std::vector<float> newSamples(
        static_cast<std::size_t>(availableSamples));
    ...
    if (availableSamples >= analysisWindowSize)
    {
        ...
        std::copy(newSamples.end() - analysisWindowSize,
                  newSamples.end(),
    ...
    std::move(analysisBuffer.begin() + availableSamples,
              analysisBuffer.end(),
              analysisBuffer.begin());
    std::copy(newSamples.begin(),
              newSamples.end(),
              analysisBuffer.end() - availableSamples);
}
...
float TunerComponent::calculateInputLevel() const
{
    ...
    return static_cast<float>(
        std::sqrt(squareSum / static_cast<double>(analysisWindowSize)));
}
...
double TunerComponent::detectPitch() const
{
    ...
    const auto minimumLag = std::max(
        2,
        static_cast<int>(sampleRate / maximumDetectableFrequencyHz));
    ...
    for (int lag = minimumLag; lag <= maximumLag; ++lag)
    {
        ...
        {
            const auto firstSample = static_cast<double>(
                analysisBuffer[static_cast<std::size_t>(index)]);
            const auto delayedSample = static_cast<double>(
...
double TunerComponent::smoothFrequency(double frequency)
{
    ...
    recentMidiPitches[static_cast<std::size_t>(recentPitchWriteIndex)] =
        midiPitch;
    recentPitchWriteIndex =
        (recentPitchWriteIndex + 1) % maximumAverageWindow;
    recentPitchCount =
        std::min(recentPitchCount + 1, maximumAverageWindow);
    ...
    smoothedMidiNote = smoothedMidiNote == 0.0
        ? averagedMidiPitch
        : smoothedMidiNote
            + easing * (averagedMidiPitch - smoothedMidiNote);
...
double TunerComponent::averageRecentMidiPitches() const
{
    const auto requestedWindow =
        static_cast<int>(averagingSlider.getValue());
    const auto pitchesToAverage =
        std::min(requestedWindow, recentPitchCount);

    double sum = 0.0;
    for (int offset = 0; offset < pitchesToAverage; ++offset)
    {
        const auto index =
            (recentPitchWriteIndex - 1 - offset + maximumAverageWindow)
            % maximumAverageWindow;
...
double TunerComponent::midiToFrequency(double midiPitch)
{
    return referenceFrequencyHz
        * std::pow(2.0, (midiPitch - 69.0) / 12.0);
}
...
void TunerComponent::updateDisplayedNote(double frequency)
{
    ...
    else if (std::abs(midiPitch - static_cast<double>(lockedMidiNote))
    ...
    displayedCents =
        100.0 * (midiPitch - static_cast<double>(lockedMidiNote));
}
...
void TunerComponent::addHistoryPoint(double midiPitch)
{
    ...
    const auto desiredPointCount = juce::jlimit(
        100,
        maximumGraphPoints,
        static_cast<int>(durationSlider.getValue()
                         * analysisRefreshRateHz));

    const auto excessPointCount =
        static_cast<int>(graphHistory.size()) - desiredPointCount;
    if (excessPointCount > 0)
    {
        graphHistory.erase(graphHistory.begin(),
                           graphHistory.begin() + excessPointCount);
    }
}
                                    ...
                                    juce::Rectangle<int> bounds) const
{
    ...
    double minimumValue = hasLockedMidiNote
        ? static_cast<double>(lockedMidiNote) - 3.0
        : 66.0;
    ...
    {
        minimumValue =
            *std::min_element(validValues.begin(), validValues.end()) - 0.5;
        maximumValue =
            *std::max_element(validValues.begin(), validValues.end()) + 0.5;
                                  ...
                                  juce::Rectangle<int> bounds) const
{
    ...
    const auto indicatorColour =
        std::abs(displayedCents) <= inTuneToleranceCents
            ? palette.inTune
            : palette.accent;
    ...
    graphics.fillRoundedRectangle(
        barBounds.toFloat().withHeight(10.0f).withCentre(
            { static_cast<float>(barBounds.getCentreX()),
    ...
    for (const auto cents : { -50, -25, 0, 25, 50 })
    {
        ...
        graphics.setColour(
            cents == 0 ? palette.foreground : palette.muted);
    ...
    graphics.fillEllipse(indicatorX - 10.0f,
                                    ...
                                    juce::Rectangle<int> bounds) const
{
    ...
    for (int step = 0; step <= 64; ++step)
    {
        ...
        const auto angle =
            startAngle + proportion * (endAngle - startAngle);
    ...
    for (const auto cents : { -50, -25, 0, 25, 50 })
    {
        ...
        const auto outerPoint = centre + juce::Point<float>(
            static_cast<float>(std::cos(angle) * radius),
            static_cast<float>(std::sin(angle) * radius));
        const auto innerPoint = centre + juce::Point<float>(
            static_cast<float>(std::cos(angle) * (radius - 15.0f)),
            static_cast<float>(std::sin(angle) * (radius - 15.0f)));

        graphics.setColour(
            cents == 0 ? palette.foreground : palette.muted);
        graphics.drawLine({ innerPoint, outerPoint },
                          cents == 0 ? 2.0f : 1.0f);
    }

    const auto needleCents = hasSignal
        ? juce::jlimit(-50.0, 50.0, displayedCents)
        : 0.0;
    ...
    graphics.fillEllipse(centre.x - 7.0f,
                         centre.y - 7.0f,
                         14.0f,
                         14.0f);

    graphics.setColour(palette.muted);
    graphics.drawText("FLAT",
                      bounds.removeFromLeft(74).removeFromBottom(24),
                      juce::Justification::centred);
    graphics.drawText("SHARP",
                      bounds.removeFromRight(74).removeFromBottom(24),
                      juce::Justification::centred);
}
...
void TunerComponent::paint(juce::Graphics& graphics)
{
    ...
    graphics.drawText(displayedNote,
                      noteArea.removeFromTop(96),
                      juce::Justification::centred);

    graphics.setFont(juce::FontOptions(18.0f));
    graphics.drawFittedText(statusText(),
                            noteArea.removeFromTop(42),
                            juce::Justification::centred,
                            2);

    const auto preferredDisplayHeight = std::max(
        90,
        bounds.getHeight() - controlAreaHeight() - 8);
...
void TunerComponent::resized()
{
    ...
    bounds.removeFromTop(8);
    const auto placeSliderRow = [&bounds](juce::Label& label,
                                          juce::Slider& slider)
    {
