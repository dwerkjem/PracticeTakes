...
The application version lives only in the root `VERSION` file (CMake reads
and validates `MAJOR.MINOR.PATCH`, then propagates it into JUCE metadata and
the window title). Never hardcode or copy the version elsewhere.
